...
function Save-CacheAndOutput ([string]$FilePath, [System.Collections.Generic.List[string]]$Lines) {$CleanLines = [System.Collections.Generic.List[string]]::new()
    ...
    if ($CleanLines.Count -gt 0) { [System.IO.File]::WriteAllLines($FilePath, $CleanLines, [System.Text.Encoding]::UTF8); foreach ($cl in $CleanLines) { Write-Output$cl } }
}
function Read-CacheAndOutput ([string]$FilePath) {
    if (Test-Path $FilePath) { foreach ($l in [System.IO.File]::ReadAllLines($FilePath, [System.Text.Encoding]::UTF8)) { if (-not [string]::IsNullOrWhiteSpace($l)) { Write-Output$l } } }
}
...
if ($NeedUpdateWeekly) {$linesWeekly = [System.Collections.Generic.List[string]]::new()
    try {
        ...
        foreach ($h in$hu) { $av_reg = "$($h.PSPath)\SOFTWARE\Aomei\AnyViewer\Option"; if (Test-Path $av_reg) { $val = Get-ItemPropertyValue -Path$av_reg -Name "DeviceID" -ErrorAction SilentlyContinue; if ($val) { $ANYVIEWER_ID =$val; break } } }
        if ($ANYVIEWER_ID -eq "Not Installed") { $val = Get-ItemPropertyValue -Path "HKLM:\SOFTWARE\WOW6432Node\Aomei\AnyViewer\Option" -Name "DeviceID" -ErrorAction SilentlyContinue; if ($val) { $ANYVIEWER_ID =$val } }
