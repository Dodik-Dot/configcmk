...
# 3. Ekstraksi Host & Pencegahan Port-Doubling
...
$AgentLibFolder   = "C:\ProgramData\checkmk\agent\lib"
...
$Directories = @($AgentLocalFolder, $AgentLibFolder, $LogFolder)
...
$LhmSources = @(
    ...
    "$BaseUrl/local_checks/LibreHardwareMonitorLib.dll",
    "https://raw.githubusercontent.com/LibreHardwareMonitor/LibreHardwareMonitor/master/LibreHardwareMonitorLib/bin/Release/net472/LibreHardwareMonitorLib.dll"
)
...
foreach ($url in $LhmSources) {
    try {
        ...
        Invoke-WebRequest -Uri $url -OutFile $LhmDllPath -UseBasicParsing -ErrorAction Stop
        if ((Get-Item $LhmDllPath).Length -gt 1000) {
...
# 8. Unduh Script Local Checks dari GitHub (Tepat 10 Skrip)
$LocalChecks = @(
    "battery_health.ps1",
    "cpu_info.ps1",
